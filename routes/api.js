...
try {
var zahirr = db.get("Jeprigans");
} catch (e) {
	console.log('')
}

var creatorList = ['@Jepri','@jepribrs','@jepribarus14','@anjayjep', '@jeprigans','@jepriganz','@jepriganteng'];
...
loghandler = {
    notparam: {
        ...
        getApikey: 'apikey lu salah ngab,gak punya apikey? chat me gratis kok api nya wa.me/12347140010'
    },
    notkey: {
        status: false,
        creator: `${creator}`,
    ...
    invalidKey: {
        ...
        creator: `${creator}`,
        code: 406,
        message: 'apikey lu salah ngab,gak punya apikey? chat me gratis kok api nya wa.me/12347140010'
    },
    invalidlink: {
        status: false,
        creator: `${creator}`,
    ...
    error: {
        status: false,
        creator: `${creator}`,
        message: 'mungkin sedang dilakukan perbaikan, sabar ngab'
    }
}
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------JepriGanteng'+'Jepri--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'jeprisyg') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'jeprisyg') return res.json(loghandler.invalidKey)
...
router.get('/textmaker/alay', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'jeprisyg') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'blackpink' && theme != 'glass') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/textmaker/water', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'jeprisyg') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'waterlove' && theme != 'beach') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/textmaker/pantai', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'jeprisyg') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'sansumer' && theme != 'oldtext') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
